...
import {
  downloadCoupangCategoryBatchWorkbook,
  downloadCoupangCategoryTemplateWorkbook,
  importCoupangCategoryWorkbook,
} from "./excel";
import { requestCoupangCategoryPrediction } from "./endpoints";
import { sanitizeCoupangCategoryPredictParams } from "./params";
import type {
  CoupangCategoryBatchPredictResult,
  CoupangCategoryBatchResultRow,
  CoupangCategoryPredictionCandidate,
...
const getStringField = (value: Record<string, unknown>, keys: string[]): string | null => {
...
const toCandidates = (items: unknown[]): CoupangCategoryPredictionCandidate[] => {
  return items
    ...
    .filter((candidate): candidate is CoupangCategoryPredictionCandidate => {
...
const extractItemArray = (raw: unknown): unknown[] => {
...
const getCodeAndMessage = (raw: unknown): { code: string | null; message: string | null } => {
  if (!raw || typeof raw !== "object") {
...
const normalizePredictionResponse = (raw: unknown): CoupangCategoryPredictionResult => {
...
export const predictCoupangCategory = async (
...
const getCoupangCredentials = async (): Promise<{ accessKey: string; secretKey: string }> => {
  ...
  if (!accessKey || !secretKey) {
    throw new Error(
      "Coupang Access Key 또는 Secret Key가 저장되어 있지 않습니다. 설정 페이지에서 먼저 저장해주세요."
    );
  }
...
const predictWithCredentials = async (
  payload: CoupangCategoryPredictRequest,
  accessKey: string,
  secretKey: string
): Promise<CoupangCategoryPredictionResult> => {
  const params = sanitizeCoupangCategoryPredictParams(payload);

  const raw = await requestCoupangCategoryPrediction(params, accessKey, secretKey);
...
export const downloadCoupangCategoryTemplate = async (
  payload: DownloadRequest
): Promise<{ result: boolean; errorLog: string }> => {
  return downloadCoupangCategoryTemplateWorkbook(payload);
};

export const importCoupangCategoryExcel = async (
  payload: CoupangCategoryExcelImportRequest
) => {
  return importCoupangCategoryWorkbook(payload);
};

export const predictCoupangCategoryBatch = async (
  payload: CoupangCategoryBatchPredictRequest,
  onProgress?: (progress: {
  ...
  const rows: CoupangCategoryBatchResultRow[] = [];
  let successCount = 0;
  let errorCount = 0;
...
export const downloadCoupangCategoryBatchResults = async (
  payload: CoupangCategoryBatchExportRequest
): Promise<{ result: boolean; errorLog: string }> => {
  return downloadCoupangCategoryBatchWorkbook(payload);
};
